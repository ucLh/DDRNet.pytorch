...
def DualResNet_imagenet(cfg, pretrained=False):
    model = DualResNet(BasicBlock, [3, 4, 6, 3], num_classes=19, planes=64, spp_planes=128, head_planes=256, augment=True)
    if pretrained:
...
def get_seg_model(cfg, **kwargs):
    ...
    model = DualResNet_imagenet(cfg, pretrained=False)
    return model
